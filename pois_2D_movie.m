...
movie_file_name = 'CLBM_Pois_Re100.avi';
movie_file = avifile(movie_file_name);
...
if ((run_dec ~= 'n') && (run_dec ~= 'N'))
    ...
    for ts = 1:Num_ts
        ...
        if(mod(ts,plot_freq)==0)
            ...
            figure(1)
            %hfig = figure(1);
            %set(hfig,'Position',[500 500 600 900]);
            subplot(3,1,1)
            ...
            title('\bf{Mid-channel velocity profile}');
            grid on
            ...
            frame = getframe(gcf);
            movie_file = addframe(movie_file,frame);
